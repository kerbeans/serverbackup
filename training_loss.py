...
from loss import intermediate_loss,KL_distance
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet.parameters()):
                ...
                latents_anchor=torch.cat([target_latent_zt,ref_ci],dim=1)
                latents_p=torch.cat([target_latent_zt,noise],dim=1)
                latents_input=torch.cat([latents_anchor,latents_p,latents_p],dim=0)
                encoder_hidden_states = text_encoder(batch["caption"])[0]
                
                text_guidance=torch.cat([encoder_hidden_states,encoder_hidden_states,text_encoder(zerotoken)[0]],dim=0)
                timesteps=torch.cat([timesteps]*3,dim=0)

                noise_pred=unet(latents_input,timesteps,text_guidance).sample
                pred_anchor,pred_Ci,pred_Ct=noise_pred.chunk(3,0)
                loss1 = F.mse_loss(pred_anchor,noise,reduction='none').mean([1,2,3]).mean()
                #-------------------inf----------
                loss2= KL_distance(anchor=pred_anchor.detach(),pt=pred_Ct.detach(),pi=pred_Ci)
                # loss=loss1+loss2
